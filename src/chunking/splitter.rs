...
/// Find a semantic boundary near the target position
fn find_semantic_boundary(content: &str, target_pos: usize, max_pos: usize) -> usize {
    ...
    let search_range = &content[end_pos..std::cmp::min(max_pos, content.len())];
    ...
    std::cmp::min(target_pos, content.len())
}
...
pub fn add_chunk_context(
    ...
    if let Some(parent) = parent_node {
        if let Some(parent_name_node) = find_name_node(parent) {
